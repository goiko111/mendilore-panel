  ...
  return (
    <div>
      ...
      <div className="bg-card border border-border rounded-xl p-5 mb-6">
        <div className="flex items-center justify-between gap-4 mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Sesiones, usuarios, conversiones y geo en directo desde GA4 · últimos 30 días
            </p>
          </div>
          <a
        ...
        {ga4 ? (
          <>
            {/* Fila 1: 4 KPIs principales con variación 30d */}
            {(() => {
              const varSesiones = ga4.sesionesPrev > 0 ? ((ga4.sesiones - ga4.sesionesPrev) / ga4.sesionesPrev) * 100 : null;
              const varUsuarios = ga4.usuariosPrev > 0 ? ((ga4.usuarios - ga4.usuariosPrev) / ga4.usuariosPrev) * 100 : null;
              const fmtVar = (v: number | null) => v === null ? "" : `${v > 0 ? "+" : ""}${v.toFixed(1)}%`;
              const colorVar = (v: number | null) => v === null ? "text-muted-foreground" : v > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400";
              return (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                  <div className="rounded-lg border border-border p-3">
                    ...
                    {varSesiones !== null && (
                      <div className={`text-[11px] mt-0.5 ${colorVar(varSesiones)}`}>{fmtVar(varSesiones)} vs 30d antes</div>
                    )}
                  </div>
                  <div className="rounded-lg border border-border p-3">
                    ...
                    {varUsuarios !== null && (
                      <div className={`text-[11px] mt-0.5 ${colorVar(varUsuarios)}`}>{fmtVar(varUsuarios)} vs 30d antes</div>
                    )}
                  </div>
                  <div className="rounded-lg border border-border p-3">
                    <div className="text-[11px] uppercase tracking-wide text-muted-foreground">Engagement</div>
                    <div className="text-xl font-semibold text-foreground mt-1">{formatPercent(ga4.engagementRate * 100)}</div>
                    <div className="text-[11px] mt-0.5 text-muted-foreground">Duración: {Math.round(ga4.sessionDuration)}s</div>
                  </div>
                  <div className="rounded-lg border border-border p-3">
                    ...
                    <div className="text-[11px] mt-0.5 text-muted-foreground">Rebote: {formatPercent(ga4.bounceRate * 100)}</div>
                  </div>
                </div>
              );
            })()}

            {/* Fila 2: Top páginas | Top fuentes */}
            ...
            {/* Fila 3: Dispositivos | Países | Conversiones */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 pt-4 border-t border-border">
              <div>
                <div className="text-xs font-semibold text-foreground mb-2">Dispositivos</div>
                <ul className="space-y-1.5">
                  {ga4.dispositivos.length === 0 ? (
                    <li className="text-xs text-muted-foreground italic">Sin datos</li>
                  ) : (() => {
                    const totalDev = ga4.dispositivos.reduce((s, d) => s + d.sesiones, 0);
                    return ga4.dispositivos.map((d) => {
                      const pct = totalDev > 0 ? (d.sesiones / totalDev) * 100 : 0;
                      const label = d.tipo === "mobile" ? "Móvil" : d.tipo === "desktop" ? "Escritorio" : d.tipo === "tablet" ? "Tablet" : d.tipo;
                      return (
              ...
              <div>
                <div className="text-xs font-semibold text-foreground mb-2">Top países</div>
                <ul className="space-y-1.5">
                  {ga4.paises.length === 0 ? (
                    <li className="text-xs text-muted-foreground italic">Sin datos</li>
                  ) : (
                    ga4.paises.slice(0, 6).map((p) => (
                      <li key={p.pais} className="flex items-center justify-between text-xs">
                        <span className="truncate text-foreground">{p.pais || "(unknown)"}</span>
                        <span className="shrink-0 ml-3 tabular-nums text-muted-foreground">{p.sesiones}</span>
                      </li>
                    ))
                  )}
                </ul>
              ...
              <div>
                <div className="text-xs font-semibold text-foreground mb-2">Conversiones / eventos clave</div>
                <ul className="space-y-1.5">
                  {ga4.conversiones.length === 0 ? (
                    <li className="text-xs text-muted-foreground italic">Sin eventos personalizados aún</li>
                  ) : (
                    ga4.conversiones.map((c) => (
                      <li key={c.evento} className="flex items-center justify-between text-xs">
                        <span className="truncate text-foreground font-mono text-[10px]">{c.evento}</span>
                        <span className="shrink-0 ml-3 tabular-nums text-muted-foreground">{c.total}</span>
                      </li>
                    ))
                  )}
                </ul>
